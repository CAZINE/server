...
AddEventHandler("vehtuning",function()
	...
	if IsEntityAVehicle(vehicle) then
        ...
        SetVehicleNumberPlateText(vehicle,"Nel")
...
RegisterNetEvent('vRP:deleteVehicle')
AddEventHandler('vRP:deleteVehicle', function()
    local ped = PlayerPedId()
    local vehicle = GetVehiclePedIsIn(ped, false)
    
    if vehicle ~= 0 then
        DeleteEntity(vehicle)
        vRP.notify({"Veículo deletado com sucesso!"}) -- Notificação de sucesso
    else
        vRP.notify({"Você não está em um veículo!"}) -- Notificação de erro
    end
end)
