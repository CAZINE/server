...
function vRPN.Identidade()
	...
	if user_id then
		...
		local unidade = vRPN.getUserGroupByType(user_id, "unidade")
		local cargo = vRPN.getUserGroupByType(user_id,"Cargo")
		local vip = vRPN.getUserGroupByType(user_id,"vip")
		if identity then
			return vRP.format(parseInt(cash)),vRP.format(parseInt(banco)),vRP.format(parseInt(mypaypal)),identity.name,identity.firstname,identity.age,identity.user_id,identity.registration,identity.phone,job,unidade,cargo,vip,vRP.format(parseInt(mymultas)),vRP.format(parseInt(mybills)),identity.phone
		end
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
