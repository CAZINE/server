...
cReative.ResetItemTexture = function (obj, category) {
    ...
    $.post('http://skinshop/updateSkin', JSON.stringify({
        clothingType: category, articleNumber: defaultTextureValue, type: "texture",
    }));
}
...
$(document).on('click', '.clothing-menu-option-item-right', function (e) {
    ...
    if (clothingCategory == "hair") {
        ...
        $.post('http://skinshop/updateSkin', JSON.stringify({
            clothingType: clothingCategory, articleNumber: newValue, type: buttonType,
        }));
        if (buttonType == "item") {
        ...
        if (buttonType == "item") {
            ...
            if (clothingCategory == "accessory" && newValue == 13) {
                $(inputElem).val(14);
                $.post('http://skinshop/updateSkin', JSON.stringify({
                    clothingType: clothingCategory, articleNumber: 14, type: buttonType,
                }));
            } else {
    ...
    if (buttonType == "item") {
        if (newValue >= skinData[clothingCategory].defaultItem) {
            if (clothingCategory == "accessory" && newValue == 13) {
                $(inputElem).val(12);
                $.post('http://skinshop/updateSkin', JSON.stringify({
                    clothingType: clothingCategory, articleNumber: 12, type: buttonType,
                }));
            } else {
                $(inputElem).val(newValue);
                $.post('http://skinshop/updateSkin', JSON.stringify({
...
$(document).on('change', '.item-number', function () {
    ...
    if (clothingCategory == "accessory" && inputVal == 13) {
        ...
        $.post('http://skinshop/updateSkinOnInput', JSON.stringify({
            clothingType: clothingCategory, articleNumber: parseFloat(inputVal), type: buttonType,
        }));
    }
});
